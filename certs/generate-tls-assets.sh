...
function generate_new_certs() {
    ...
    if [[ "${DRY_RUN}" -eq 1 ]]; then
        ...
        if [[ "${VERBOSE}" -ne 0 ]]; then
            ...
            echo "docker run --name \"${var_es_keystore_docker_name}\" -i -w /app elasticsearch:7.3.1 /bin/sh -c \"echo ${var_encoded_instances} | base64 -w 0 -d > /app/instances.yaml && elasticsearch-certutil cert --ca-dn CN=${USE_CN} --days ${USE_EXPIRATION_IN_DAYS} --in /app/instances.yaml --out /app/${var_es_keystore_docker_name}.zip --pass ''\""
            echo ""
        fi
        docker run --name "${var_es_keystore_docker_name}" -i -w /app elasticsearch:7.3.1 /bin/sh -c "echo ${var_encoded_instances} | base64 -w 0 -d > /app/instances.yaml && elasticsearch-certutil cert --ca-dn CN=${USE_CN} --days ${USE_EXPIRATION_IN_DAYS} --in /app/instances.yaml --out /app/${var_es_keystore_docker_name}.zip --pass ''" 2>&1
        var_last_status="$?"
    else
